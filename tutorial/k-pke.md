...
\For{$(i \is 0; \ i < k; \ i \plusplus)$} {\Comment{generate matrix $\HatA \in \RingTqKxK $}}
    \For{$(j \is 0; \ j < k; \ j \plusplus)$}
        \State{$ \HatA[i,\, j] \is \SampleNTT(\rho \, || \, j \, || \, i)$}
...
\For{$(i \is 0; \ i < k; \ i \plusplus)$}
    {\Comment{generate $\NameBold{s} \in \RingZqK$}}
    \State{$\NameBold{s}[i] \is \SamplePolyCbdEtaOne(\PrfEtaOne(\sigma, N)) $}
...
\For{$(i \is 0; \ i < k; \ i \plusplus)$}
    ...
    \State{$ \NameBold{e}[i] \is \SamplePolyCbdEtaOne(\PrfEtaOne(\sigma, N)) $}
...
% line 16
\Statex{}
\State{$ \HatNameBold{s}:\RingTqK \is \NTT(\NameBold{s}) $}
    {\Comment{run $\NTT$ $k$ times (once for each coordinate of $\NameBold{s}$)}}

% line 17
\State{$ \HatNameBold{e}:\RingTqK \is \NTT(\NameBold{e}) $}
    {\Comment{run $\NTT$ $k$ times}}

% line 18
\State{$ \HatBoldT: \RingTqK \is \HatA \circ \HatNameBold{s} + \HatNameBold{e} $}
    {\Comment{noisy linear system in NTT domain}}
    \Statex{\LComment{$\RingTqK = \RingTqKxK \circ \RingTqK + \RingTqK$}}

% line 19
\Statex{}
\State{$ \EKPKE: \mathbb{B}^{384k+32} \is \ByteEncodeQ(\HatBoldT || \, \rho) $}
    \Statex{\Comment{run $\ByteEncodeQ$ $k$ times, then append $\HatA$-seed }}

% line 20
\State{$ \DKPKE: \mathbb{B}^{384k} \is \ByteEncodeQ(\HatBoldS) $}
...
% line 2
\State {$ \HatBoldT:(\RingZq)^k \is \ByteDecodeQ(\EKPKE[0:384k])$}
    {\Comment{run $\ByteDecodeQ$ $k$ times to decode $\HatBoldT$ }}

%line 3
\State{$\rho \is \EKPKE[384k:384k+32]$} {\Comment{extract 32 bytes seed from $\EKPKE$}}
...
\For{$(i \is 0; \ i < k; \ i \plusplus)$}
    ...
    \State{$ \NameBold{y}[i] \is \SamplePolyCbdEtaOne(\PrfEtaOne(r, N)) $}
...
\For{$(i \is 0; \ i < k; \ i \plusplus)$}
    ...
    \State{$ \NameBold{e_1}[i] \is \SamplePolyCbdEtaTwo(\PrfEtaTwo(r, N)) $}
    \State{$ N \is N + 1 $}
\EndFor

%lines 17
\State{$ e_2 \is \SamplePolyCbdEtaTwo(\PrfEtaTwo(r, N)) $}
    {\Comment{sample $e_2 \in \RingZq$ from CBD}}

%lines 18
\State{$ \HatBoldY \is \NTT(\BoldY) $}
    \Comment{run $\NTT$ $k$ times}
%lines 19
\State{$ \mathbf{u} \is \NTTInv(\HatA^{\T} \circ \HatBoldY) + \Name{\mathbf{e_1}} $}
    \Comment{run $\NTTInv$ $k$ times}

%lines 20
\State{$\mu \is \DecompressOne(\ByteDecodeOne(m)) $}

%lines 21
\State{$v \is \NTTInv(\HatBoldT^{\T} \circ \HatBoldY) + e_2 + \mu$}
    \Comment{encode plaintext $m$ into polynomial $v$}

%lines 22
\State {$c_1 \is \ByteDecodeDU(\CompressDU(\Name{\mathbf{u}}))$}
\Comment{run $\ByteDecodeDU$ and $\CompressDU$ $k$ times }

%lines 23
\State {$c_2 \is \ByteDecodeDV(\CompressDV(v))$}

%lines 24
\State {\Return{$c \is (c_1||c_2)$}}
...
% line 1
\State {$c_1 \is c[0 : 32 d_u k] $}

%line 2
\State {$c_2 \is c[32 d_u k : 32 (d_u k + d_v)] $}

%line 3
\State{$ \NameBold{u}' \is \DecompressDU(\ByteDecodeDU(c_1)) $}

%line 4
\State{$ v' \is \DecompressDV(\ByteDecodeDV(c_2)) $}

%line 5
\State{$ \HatBoldS \is \ByteDecodeQ(\DKPKE) $}
    \Comment{run $\ByteDecodeQ$ $k$ times}

%line 6
\State{$ w \is v' - \NTTInv(\HatBoldS^{\T} \circ \NTT(\NameBold{u'})) $}
    \Comment{run $\NTT$ $k$ times; run $\NTTInv$ once}
%line 7
\State{$ m \is \ByteEncodeOne(\CompressOne(w))$}
