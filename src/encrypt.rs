...
impl EncryptionKey {
    #[inline(always)]
    fn _sample_small_vector_eta2_(r: [u8; 32], n: u8, mut s: &mut Poly) {
        sampler::sample_secret_eta2(r, n, s)
    }
    ...
    #[allow(unused)]
    #[inline(always)]
    pub fn decode_then_decompress_message(m: [u8; 32]) -> Poly {
        let mut md = [0u16; N];
    ...
    pub fn encrypt(&self, m: [u8; 32], r: [u8; 32]) -> [u8; 32 * (DU * K as u8 + DV) as usize] {
...

        {
            ...
            pub fn mod_pm_q(a: i32, b: i32) -> u32 {
                ...
                if diff > HALF_Q { QU32 - diff } else { diff }
            }
            ...
            for i in 0..K {
                ...
                for i in 0..N {
                    let _error_ =
                        mod_pm_q(decompress::<DU>(_c1_coeff_[i] as u16) as i32, _u_coeff_[i]);
