...
use crate::ntt::NTT;
use crate::params::{DU, DV, N, Q, QI32, K};
...
impl RingElement {
    ...
    pub fn byte_encode_1(&self) -> [u8; 32] {
        let mut arr = [0; 32];
        byte_encode(self.into(), &mut arr, 1);
        arr
    }
...
impl Poly {
    ...
    pub fn compress_1(&self) -> Poly {
        Poly::from(&self.c.map(|c| compress_1(c as u16)))
    }

    pub fn decompress<const D: u8>(&self) -> Poly {
        Poly::from(&self.c.map(|c| decompress::<D>(c as u16)))
    }
    ...
    pub fn sample_poly_cbd_eta_3(&mut self, b: &[u8; 64 * 3]) {
        for i in 0..N / 4usize {
            // read 24 bits
            let mut w: u32 =
                b[i * 3 + 0] as u32 | ((b[i * 3 + 1] as u32) << 8) | ((b[i * 3 + 2] as u32) << 16);
            // use 6 bits for each coefficient
            for j in 0..4 {
                let x = ((w >> 0) & 1) + ((w >> 1) & 1) + ((w >> 2) & 1);
                let y = ((w >> 3) & 1) + ((w >> 4) & 1) + ((w >> 5) & 1);
                self.c[i * 4 + j] =
                    FieldElement::sub(&FieldElement::from(x as i32), &FieldElement::from(y as i32))
                        .into();
                w >>= 6;
            }
        }
...
#[allow(unused)]
pub fn inner_product(a: [RingElement; K], b: [RingElement; K]) -> Poly {
